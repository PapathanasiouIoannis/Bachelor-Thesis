...
def obj_xgb(trial, X_fit, y_fit, groups_fit):
    ...
    for tr_idx, val_idx in sgkf.split(X_fit, y_fit, groups=groups_fit):
...
def train_model(df):
    ...
    temp_idx, test_idx = next(sgkf_test.split(df_clean, y, groups=groups))

    df_temp = df_clean.iloc[temp_idx]
    y_temp = y.iloc[temp_idx]
    groups_temp = groups.iloc[temp_idx]

    sgkf_calib = StratifiedGroupKFold(n_splits=5, shuffle=True, random_state=42)
    fit_idx, calib_idx = next(sgkf_calib.split(df_temp, y_temp, groups=groups_temp))
    ...
    for model_name, cols in feature_sets.items():
        ...
        # Rigorous Split: Train on X_fit (-1), Calibrate on X_calib (0)
        test_fold = np.concatenate([np.full(len(X_fit), -1), np.zeros(len(X_calib))])
        ps = PredefinedSplit(test_fold)
        
        X_fit_calib = pd.concat([X_fit, X_calib], axis=0).reset_index(drop=True)
        y_fit_calib = pd.concat([y_fit, y_calib], axis=0).reset_index(drop=True)

        cal_clf = CalibratedClassifierCV(ensemble_clf, method="isotonic", cv=ps)
        cal_clf.fit(X_fit_calib, y_fit_calib)
