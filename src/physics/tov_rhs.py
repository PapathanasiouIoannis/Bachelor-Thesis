...
_P_MIN_SAFE = CONSTANTS["TOV_P_MIN_SAFE"]
_G_CONV = CONSTANTS["G_CONV"]
_A_CONV = CONSTANTS["A_CONV"]
...
def tov_rhs(r, y_state, eos_callable):
    ...
    term_2 = m + (r**3 * P_safe * _G_CONV)

    # A_CONV converts solar masses to km (G*M_sun/c^2)
    term_3 = r * (r - 2.0 * m * _A_CONV)
    ...
    # F term
    F = (
        1.0 - _A_CONV * _G_CONV * (r**2) * (epsilon - P_safe)
    ) * exp_lambda
